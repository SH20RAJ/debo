...
var Ai = I((Dh, _i) => {
  ...
  var Aa = a(function(r) {
    ...
    r(20, vi), r(21, Xt), r(23, Xt), r(26, Xt), r(700, parseFloat), r(701, parseFloat), r(16, Ei), r(1082, lt), r(1114, lt), r(1184, lt), r(600, er), r(651, ne), r(718, Ia), r(1000, Ea), r(1001, Ta), r(1005, Yt), r(1007, Yt), r(1028, Yt), r(1016, _a), r(1017, Aa), r(1021, Zt), r(1022, Zt), r(1231, Zt), r(1014, ne), r(1015, ne), r(1008, ne), r(1009, ne), r(1040, ne), r(1041, ne), r(1115, Jt), r(1182, Jt), r(1185, Jt), r(1186, Si), r(1187, Ca), r(17, xi), r(114, JSON.parse.bind(JSON)), r(3802, JSON.parse.bind(JSON)), r(199, bi), r(3807, bi), r(3907, ne), r(2951, ne), r(791, ne), r(1183, ne), r(1270, ne);
  }, "init");
  _i.exports = { init: Pa };
});
var Ti = I((Oh, Ci) => {
...
__export(exports_schema, {
  ...
  documents: () => documents,
  decisions: () => decisions,
  deboMailThreads: () => deboMailThreads,
  deboMailParticipants: () => deboMailParticipants,
  deboMailMessages: () => deboMailMessages,
  deboMailAddresses: () => deboMailAddresses,
  connectorSyncRuns: () => connectorSyncRuns,
...
var sourceFiles = pgTable("source_files", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
...
var memoryChunks = pgTable("memory_chunks", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
  sourceId: text("source_id").notNull().references(() => sources.id),
...
var memoryItems = pgTable("memory_items", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
  sourceId: text("source_id").notNull().references(() => sources.id),
...
var personMentions = pgTable("person_mentions", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
...
var projectLinks = pgTable("project_links", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
...
var decisions = pgTable("decisions", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
  sourceId: text("source_id").references(() => sources.id),
  ...
  reason: text("reason"),
  status: text("status", { enum: ["active", "changed", "deprecated"] }).notNull().default("active"),
  confidence: real("confidence"),
...
var memoryRelations = pgTable("memory_relations", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
...
var answerCitations = pgTable("answer_citations", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
...
var connectorAccounts = pgTable("connector_accounts", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
  provider: text("provider", {
...
var connectorSyncRuns = pgTable("connector_sync_runs", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
  connectorAccountId: text("connector_account_id").notNull().references(() => connectorAccounts.id),
  status: text("status", {
...
var deboMailAddresses = pgTable("debo_mail_addresses", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
...
var deboMailThreads = pgTable("debo_mail_threads", {
  id: text("id").primaryKey(),
  workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
...
var deboMailMessages = pgTable("debo_mail_messages", {
  id: text("id").primaryKey(),
  threadId: text("thread_id").notNull().references(() => deboMailThreads.id),
...
var deboMailParticipants = pgTable("debo_mail_participants", {
  id: text("id").primaryKey(),
  threadId: text("thread_id").notNull().references(() => deboMailThreads.id),
  userId: text("user_id").notNull().references(() => users.id),
  address: text("address").notNull(),
...
_e.fetchFunction = async (input, init) => {
...
async function resolveSession() {
  ...
  let userId = process.env.DEBO_USER_ID || config2.userId;
  let workspaceId = process.env.DEBO_WORKSPACE_ID || config2.workspaceId;
  if (!userId) {
  ...
  if (!workspaceId) {
    const [firstWS] = await db.select().from(workspaces).where(eq(workspaces.ownerUserId, userId)).limit(1);
    if (firstWS) {
...
function chunkText(text2) {
  ...
  for (const p2 of paragraphs) {
    ...
    if (buf.length + p2.length + 2 <= TARGET_CHUNK_CHARS) {
      buf = `${buf}
...
program2.command("search").description("Semantic vector query search across private memories and files").argument("<query>", "Semantic search text").option("-l, --limit <number>", "Maximum result results to return", "5").action(async (query, options) => {
  try {
    ...
    console.log(import_picocolors.default.blue(`\u2139 Searching private memory graph for "${normalizedQuery}"...`));
    ...
    }).from(memoryChunks).innerJoin(sources, eq(memoryChunks.sourceId, sources.id)).where(and(eq(memoryChunks.userId, userId), ilike(memoryChunks.text, `%${normalizedQuery}%`))).orderBy(desc(memoryChunks.createdAt)).limit(limit);
    if (rows.length === 0) {
      console.log(import_picocolors.default.yellow("\u26A0 No matching memories found in database."));
      return;
    }
    console.log(import_picocolors.default.green(`\u2714 Found ${rows.length} relevant context matches:
`));
    rows.forEach((row, i) => {
      console.log(import_picocolors.default.bold(import_picocolors.default.cyan(`${i + 1}. [${row.sourceType.toUpperCase()}] ${row.title || "Untitled"}`)));
      console.log(import_picocolors.default.dim(`   ID: ${row.sourceId} | Date: ${row.createdAt}`));
...
`).join(`
   > `)}`);
    ...
    console.error(import_picocolors.default.red(`\u2716 Search failed: ${err.message}`));
  }
});
var journal = program2.command("journal").description("Manage personal journal entries (CRUD)");
journal.command("list").description("List all journal entries").action(async () => {
  try {
    const { userId } = await resolveSession();
    const rows = await db.select().from(sources).where(and(eq(sources.userId, userId), eq(sources.type, "journal"))).orderBy(desc(sources.createdAt));
    if (rows.length === 0) {
      console.log(import_picocolors.default.yellow("\u2139 No journal entries found. Use 'debo journal create' to write one."));
      return;
    }
    console.log(import_picocolors.default.bold(import_picocolors.default.green(`
=== Journal Entries List (${rows.length}) ===`)));
    rows.forEach((row) => {
      console.log(import_picocolors.default.cyan(`\u2022 [${row.id}] ${row.title || "Untitled"}`));
      console.log(import_picocolors.default.dim(`  Date: ${row.createdAt} | Status: ${row.status}`));
    });
    console.log();
  } catch (err) {
    console.error(import_picocolors.default.red(`\u2716 List journals failed: ${err.message}`));
  }
});
journal.command("show").description("View the full text content of a journal entry").argument("<journalId>", "ID of the journal entry").action(async (journalId) => {
  try {
    const { userId } = await resolveSession();
    const [entry] = await db.select().from(sources).where(and(eq(sources.id, journalId), eq(sources.userId, userId), eq(sources.type, "journal"))).limit(1);
    if (!entry) {
      console.error(import_picocolors.default.red(`\u2716 Journal entry not found for ID: ${journalId}`));
      return;
    }
    console.log(import_picocolors.default.bold(import_picocolors.default.cyan(`
Title: ${entry.title || "Untitled"}`)));
    console.log(import_picocolors.default.dim(`ID: ${entry.id} | Date: ${entry.createdAt}
`));
    console.log(import_picocolors.default.white(entry.plainText || "No content."));
    console.log();
  } catch (err) {
    console.error(import_picocolors.default.red(`\u2716 Show journal failed: ${err.message}`));
  }
});
journal.command("create").description("Create a new journal entry").requiredOption("-t, --title <title>", "Journal entry title").requiredOption("-c, --content <content>", "Journal entry text body").action(async (options) => {
  try {
    ...
    if (chunks.length > 0) {
      const chunkRows = chunks.map((chunk, index2) => ({
        id: newId("chk"),
        userId,
        workspaceId,
        sourceId,
    ...
    console.log(import_picocolors.default.green(`\u2714 Created journal entry: [${created.id}] "${created.title}"`));
    console.log(import_picocolors.default.dim(`  Indexed ${chunks.length} text chunks successfully.`));
  } catch (err) {
    console.error(import_picocolors.default.red(`\u2716 Create journal failed: ${err.message}`));
  }
});
journal.command("update").description("Update an existing journal entry").argument("<journalId>", "ID of the journal entry").option("-t, --title <title>", "New title").option("-c, --content <content>", "New content text body").action(async (journalId, options) => {
  try {
    const { userId, workspaceId } = await resolveSession();
    const [entry] = await db.select().from(sources).where(and(eq(sources.id, journalId), eq(sources.userId, userId), eq(sources.type, "journal"))).limit(1);
    if (!entry) {
    ...
    if (options.content) {
      ...
      console.log(import_picocolors.default.green(`\u2714 Re-indexed ${chunks.length} text chunks.`));
    }
    console.log(import_picocolors.default.green(`\u2714 Updated journal entry: [${journalId}] "${updatedTitle}"`));
  } catch (err) {
    console.error(import_picocolors.default.red(`\u2716 Update journal failed: ${err.message}`));
  }
});
journal.command("delete").description("Delete a journal entry").argument("<journalId>", "ID of the journal entry").action(async (journalId) => {
  try {
    const { userId } = await resolveSession();
    await db.delete(memoryChunks).where(eq(memoryChunks.sourceId, journalId));
    const res = await db.delete(sources).where(and(eq(sources.id, journalId), eq(sources.userId, userId), eq(sources.type, "journal")));
    console.log(import_picocolors.default.green(`\u2714 Successfully deleted journal entry: [${journalId}]`));
  } catch (err) {
    console.error(import_picocolors.default.red(`\u2716 Delete journal failed: ${err.message}`));
  }
});
var task = program2.command("task").description("Manage tasks & action items");
task.command("list").description("List all active tasks").action(async () => {
  try {
    const { userId } = await resolveSession();
    const rows = await db.select().from(tasks).where(and(eq(tasks.userId, userId), ilike(tasks.status, "%"))).orderBy(desc(tasks.createdAt));
    if (rows.length === 0) {
      console.log(import_picocolors.default.yellow("\u2139 No tasks found. Use 'debo task create' to add one."));
      return;
    }
    console.log(import_picocolors.default.bold(import_picocolors.default.green(`
=== Task List (${rows.length}) ===`)));
    rows.forEach((row) => {
      ...
      const statusColor = row.status === "done" ? import_picocolors.default.green : row.status === "doing" ? import_picocolors.default.blue : import_picocolors.default.white;
      console.log(statusColor(`${check} [${row.id}] ${row.title} (${row.status.toUpperCase()})`));
      if (row.description)
        console.log(import_picocolors.default.dim(`  Details: ${row.description}`));
    });
    console.log();
  } catch (err) {
    console.error(import_picocolors.default.red(`\u2716 List tasks failed: ${err.message}`));
  }
});
task.command("create").description("Create a new task").argument("<title>", "Task summary").option("-d, --desc <description>", "Detailed description").action(async (title, options) => {
  try {
    const { userId, workspaceId } = await resolveSession();
    ...
    console.log(import_picocolors.default.green(`\u2714 Created task: [${created.id}] "${created.title}"`));
  } catch (err) {
    console.error(import_picocolors.default.red(`\u2716 Create task failed: ${err.message}`));
  }
});
task.command("complete").description("Mark an active task as complete").argument("<taskId>", "ID of the task").action(async (taskId) => {
  try {
    const { userId } = await resolveSession();
    ...
    console.log(import_picocolors.default.green(`\u2714 Marked task [${taskId}] as COMPLETED.`));
  } catch (err) {
    console.error(import_picocolors.default.red(`\u2716 Complete task failed: ${err.message}`));
  }
});
program2.command("connector").description("List status of all connected synchronization integration accounts").action(async () => {
  try {
    const { userId } = await resolveSession();
    const rows = await db.select().from(connectorAccounts).where(eq(connectorAccounts.userId, userId));
    if (rows.length === 0) {
      console.log(import_picocolors.default.yellow("\u2139 No external connectors configured. Connect apps via the dashboard web UI."));
      return;
    }
    console.log(import_picocolors.default.bold(import_picocolors.default.green(`
=== External Connectors (${rows.length}) ===`)));
    rows.forEach((row) => {
      ...
      console.log(import_picocolors.default.dim(`  Status: ${row.status.toUpperCase()} | Last Synced: ${row.lastSyncedAt || "Never"}`));
    });
    console.log();
  } catch (err) {
    console.error(import_picocolors.default.red(`\u2716 List connectors failed: ${err.message}`));
  }
});
var mcp = program2.command("mcp").description("Model Context Protocol server local commands");
mcp.command("install").description("Automatically install and register the Debo MCP server in Claude Desktop").action(async () => {
  try {
    const config2 = loadConfig();
    const userId = process.env.DEBO_USER_ID || config2.userId;
    const databaseUrl2 = process.env.DATABASE_URL;
    if (!databaseUrl2) {
      console.warn(import_picocolors.default.yellow("\u26A0 WARNING: DATABASE_URL is not set in environment. Stdio MCP might fail unless config is set."));
    }
    console.log(import_picocolors.default.blue("\u2139 Registering Debo MCP inside Claude Desktop configuration..."));
    const claudeConfigDir = path2.join(os2.homedir(), "Library", "Application Support", "Claude");
    const configPath = path2.join(claudeConfigDir, "claude_desktop_config.json");
    if (!fs2.existsSync(claudeConfigDir)) {
      fs2.mkdirSync(claudeConfigDir, { recursive: true });
    }
    let jsonConfig = { mcpServers: {} };
    if (fs2.existsSync(configPath)) {
      try {
        jsonConfig = JSON.parse(fs2.readFileSync(configPath, "utf-8"));
      } catch {}
    }
    ...
    jsonConfig.mcpServers.debo = {
      ...
      cwd: path2.resolve(__dirname, "../../mcp"),
      env: {
        DATABASE_URL: databaseUrl2 || "",
        DEBO_USER_ID: userId || ""
      }
    };
    fs2.writeFileSync(configPath, JSON.stringify(jsonConfig, null, 2), "utf-8");
    console.log(import_picocolors.default.green(`\u2714 Successfully installed Claude Desktop configuration at:
  ${configPath}`));
    console.log(import_picocolors.default.cyan(`
\u2139 To connect Cursor, register a Stdio command under Settings -> Models -> MCP:`));
    console.log(import_picocolors.default.white(`  Command: bun run ${path2.resolve(__dirname, "../../mcp/src/index.ts")}`));
    console.log(import_picocolors.default.white(`  Env variables: DATABASE_URL, DEBO_USER_ID`));
  } catch (err) {
    console.error(import_picocolors.default.red(`\u2716 MCP Installation failed: ${err.message}`));
  }
});
