...
| In-app agent actions later         | CopilotKit                                    |
| Agent/backend AI                   | **Mastra**                                    |
...
| ORM                                | Drizzle                                       |
| Vector search                      | Cloudflare Vectorize first, Turbopuffer later |
...
Vectorize is Cloudflare’s globally distributed vector database for AI apps and semantic search, so it fits nicely with your R2/Cloudflare direction. ([Cloudflare Docs][10])
...
Use **CopilotKit** later if you want Debo’s AI to control UI state, generate custom UI, edit pages, or act like an in-app copilot. CopilotKit is designed for agent frontends, generative UI, and connecting agents into React apps. ([docs.copilotkit.ai][15])

## 11. Agent backend

Use **Mastra**.

Mastra is a TypeScript/JavaScript agent framework with agents, workflows, RAG, memory, tools, and observability patterns. It fits Debo better than writing everything directly with AI SDK. ([mastra.ai][16])
...
connector-action-agent
document-summary-agent
voice-call-agent bridge
```

AI SDK can still be used underneath, but don’t make raw AI SDK your main architecture.
...
For best long-term control:

```txt
Better Auth
```

I would pick **Better Auth** for Debo because Debo is a privacy/memory product. Better Auth is TypeScript-first, framework-agnostic, plugin-based, and supports advanced auth features like 2FA, passkeys, multi-tenancy, sessions, and SSO-style patterns. ([Better Auth][21])

But if you want to launch extremely fast, Clerk is easier because it gives prebuilt Next.js components and helpers. ([Clerk][22])
...
[15]: https://docs.copilotkit.ai/?utm_source=chatgpt.com "CopilotKit Docs"
[16]: https://mastra.ai/?utm_source=chatgpt.com "Mastra: TypeScript AI Agent Framework & Platform"
