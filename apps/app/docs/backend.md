...
apps/livekit      Realtime voice/call agent
packages/db       Database schema and queries
packages/memory   Memory engine
packages/ai       Agents, prompts, tools, retrieval
...
Vector DB: Cloudflare Vectorize first, Turbopuffer later
Jobs: Trigger.dev
...
Memory Engine
  ↓
Postgres + Vectorize + Mem0
  ↓
Ask Debo retrieval pipeline
  ↓
Mastra agent generates source-backed answer
  ↓
...
`vector_id` points to Vectorize/Turbopuffer.

Do not store embeddings in Postgres if using Vectorize as the vector store.
...
Each chunk gets embedded and stored in Vectorize.
...
Apply filters if provided
  ↓
Retrieve candidates from Vectorize
  ↓
...
```txt
Frontend: Vercel
API: Railway/Fly.io/Render or Vercel serverless initially
Worker: Trigger.dev
Database: Neon
Storage: Cloudflare R2
...
```txt
Frontend: Vercel
API: Fly.io or Railway persistent Node service
Worker: Trigger.dev dedicated
Voice Agent: Fly.io/Railway container
Database: Neon with branching/backups
Storage: R2
Vector: Vectorize/Turbopuffer
```
...
API classifies intent as memory_recall
  ↓
Search Vectorize for Raj + promise
  ↓
