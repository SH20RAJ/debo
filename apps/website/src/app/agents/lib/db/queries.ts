...
import { chat, chatEvent } from "@debo/db/schema";
...
export async function getChatForUser(chatId: string, userId: string): Promise<ActiveChat | null> {
  ...
  const eventValues = events.map((eventRow) => eventRow.event) as any;
  ...
  const hasCurrentTurnCompleted = Boolean(
    ...
    events.some(
      (eventRow) =>
        ...
        isChatTurnSettledEvent(eventRow.event as any),
    ),
  );
  ...
  return {
    ...
    session: (row.eveSession ?? undefined) as any,
    title: row.title,
  };
}
