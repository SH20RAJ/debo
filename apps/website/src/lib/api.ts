...
export const api = {
  sources: {
    ...
    update: (id: string, data: { title?: string; content?: string; description?: string; status?: string; metadataJson?: string; slug?: string; privacyLevel?: string }) =>
  ...
  journal: {
    ...
    update: (id: string, data: { title?: string; content?: string; metadataJson?: string; slug?: string; privacyLevel?: string }) =>
      fetchApi(`/api/sources/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  },
  tasks: {
  ...
  media: {
    upload: async (file: File, type?: string, voiceSessionId?: string) => {
      const token = await getAccessToken();
      const fd = new FormData();
      fd.append("file", file);
      if (type) fd.append("type", type);
      if (voiceSessionId) fd.append("voiceSessionId", voiceSessionId);
      const headers: Record<string, string> = {};
      if (token) headers["x-stack-access-token"] = token;
