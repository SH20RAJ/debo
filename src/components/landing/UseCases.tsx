  ...
  return (
    <section className="py-24 md:py-32 px-6 bg-landing-surface border-t border-landing-border-light">
      <div className="mx-auto max-w-[1120px] space-y-20">
        <h2 className="text-center font-heading text-3xl md:text-4xl lg:text-[40px] font-semibold tracking-tight text-landing-text-primary max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {scenarios.map((s, i) => (
            <div key={i} className="p-8 border border-landing-border bg-landing-bg rounded-[20px]">
              <h3 className="text-[18px] font-semibold text-landing-text-primary mb-3">{s.title}</h3>
              <p className="text-[15px] font-medium text-landing-text-secondary leading-relaxed">{s.text}</p>
            </div>
          ))}
        </div>
      </div>
